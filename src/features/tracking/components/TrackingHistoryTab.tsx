...
const TrackingHistoryTab: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      setPlaybackIndex(0);
      try {
        const data = await trackingApi.getDriverHistory(
          selectedDriverId,
          selectedDate,
        );
        setHistory(data);
      ...
      {isLoading ? (
        <div className="h-[600px] bg-white rounded-3xl border border-silver/50 shadow-sm flex items-center justify-center flex-col gap-3">
          ...
          <p className="text-xs font-bold text-charcoal/40">
            Fetching tracking history trail...
          </p>
        </div>
            ...
            <div className="bg-white p-6 rounded-[28px] border border-silver/50 shadow-sm space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-silver/5 border border-silver/40 rounded-2xl">
                  ...
                  <p className="text-lg font-black text-charcoal mt-1 tracking-tight">
                    {history.total_distance_km} km
                  </p>
                </div>
                <div className="p-4 bg-silver/5 border border-silver/40 rounded-2xl">
                  <p className="text-[9px] font-bold text-charcoal/40 uppercase">
                  ...
                  <p className="text-lg font-black text-charcoal mt-1 tracking-tight">
                    {history.active_duration_minutes} mins
                  </p>
                </div>
                <div className="p-4 bg-silver/5 border border-silver/40 rounded-2xl">
                  <p className="text-[9px] font-bold text-charcoal/40 uppercase">
                  ...
                  <p className="text-lg font-black text-charcoal mt-1 tracking-tight">
                    {history.average_speed_kmh} km/h
                  </p>
                </div>
                <div className="p-4 bg-silver/5 border border-silver/40 rounded-2xl">
                  <p className="text-[9px] font-bold text-charcoal/40 uppercase">
              ...
              <div className="bg-gradient-to-br from-[#1C1C1E] to-[#2C2C2E] p-6 rounded-[28px] border border-charcoal shadow-xl text-white space-y-4">
                ...
                <div className="space-y-3 font-mono">
                  <div className="flex justify-between items-center text-xs py-1 border-b border-white/5">
                    <span className="text-white/40 font-sans font-bold">
                      Timestamp:
                    </span>
                  ...
                  <div className="flex justify-between items-center text-xs py-1 border-b border-white/5">
                    ...
                    <span
                      className={`font-bold ${currentPlaybackPoint.battery_level && currentPlaybackPoint.battery_level < 20 ? "text-rose-400 animate-pulse" : "text-cyan-400"}`}
                    >
                  ...
                  <div className="flex justify-between items-center text-xs py-1">
                    ...
                    <span className="font-bold text-white/70">
                      ±{currentPlaybackPoint.accuracy || 5}m
                    </span>
                  </div>
                </div>

                <div className="bg-white/5 p-3.5 rounded-2xl border border-white/5 space-y-1.5">
                  <span className="text-[8px] font-sans font-bold text-white/30 uppercase tracking-widest block">
                  ...
                  <p className="text-[10px] text-white/80 font-bold leading-relaxed flex items-center gap-1.5">
                    ...
                    {currentPlaybackPoint.lat.toFixed(5)},{" "}
                    {currentPlaybackPoint.lng.toFixed(5)}
                  </p>
                </div>
              </div>
            )}
