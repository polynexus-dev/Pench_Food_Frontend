...
interface DeleteBottleTypeModalProps {
  deletingContainer: BottleType | null;
  onClose: () => void;
  onSuccess: () => void;
  setNotification: (val: { title: string; message: string; type: "success" | "error" } | null) => void;
}

export const DeleteBottleTypeModal: React.FC<DeleteBottleTypeModalProps> = ({
  deletingContainer,
  onClose,
  onSuccess,
  setNotification,
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleteError(null);
    try {
      await inventoryApi.deleteBottleType(deletingContainer.id);
      setNotification({
        title: "Container Type Deleted",
        message: `Successfully deleted container type "${deletingContainer.name}".`,
        type: "success",
      });
      onSuccess();
      onClose();
      ...
      setDeleteError(
        "Failed to delete container type. It may be in use by products.",
      );
    } finally {
      ...
      <div className="bg-white rounded-3xl border border-silver/60 shadow-xl max-w-sm w-full overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="bg-red-500 text-white p-5 flex items-center justify-between">
          ...
          <button
            ...
            onClick={onClose}
            className="p-1 rounded-full hover:bg-white/10 text-white/80 hover:text-white transition-colors cursor-pointer disabled:opacity-30 disabled:pointer-events-none"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-xs font-semibold text-charcoal">
                Are you sure you want to permanently delete the container type{" "}
                <strong className="text-charcoal font-black">
                  {deletingContainer.name}
                </strong>{" "}
                from the catalog?
              </p>
              <p className="text-[11px] text-charcoal/50 font-medium mt-1">
                Deposit:{" "}
                <code className="font-mono font-bold bg-silver/30 px-1 py-0.5 rounded">
                  ₹{parseFloat(deletingContainer.deposit_amount).toFixed(2)}
                </code>
              </p>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-silver/40 flex items-center justify-end gap-2">
            <button
              ...
              disabled={isDeleteSubmitting}
              onClick={onClose}
              className="px-4 py-2 bg-silver/20 rounded-xl text-xs font-bold text-charcoal/70 hover:bg-silver/40 transition-all cursor-pointer disabled:opacity-50 disabled:pointer-events-none"
            >
              Cancel
            </button>
            <button
...
export default DeleteBottleTypeModal;
