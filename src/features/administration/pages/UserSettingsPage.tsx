...
const UserSettingsPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "profile" | "security" | "preferences" | "companies" | "system"
  >("profile");
  ...
  return (
    <div className="max-w-8xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-black text-charcoal tracking-tight">
              {activeTab === "system" ? "System Settings" : "Account Settings"}
            </h1>
            <p className="text-charcoal/50 font-medium text-xs mt-0.5">
              {activeTab === "system"
                ...
                : "Personalize your profile credentials, change security configurations, and configure application preferences."}
            </p>
          </div>
        </div>
      ...
      {/* Tabs Navigation */}
      <div className="flex flex-wrap gap-2 p-1.5 bg-silver/20 rounded-2xl mb-8 border border-silver/30 max-w-2xl md:max-w-3xl">
        <button
          onClick={() => {
      ...
      <div className="tab-content">
        ...
        {!isLockedRole && activeTab === "companies" && <CompaniesTab setError={setError} />}
        {!isLockedRole && activeTab === "system" && <SystemSettingsTab setError={setError} />}
      </div>
    </div>
  );
};
